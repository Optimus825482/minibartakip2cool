...
const CACHE_VERSION = 'minibar-v1.0.0';
...
self.addEventListener('activate', event => {
  ...
  event.waitUntil(
    caches.keys().then(cacheNames => {
      return Promise.all(
        cacheNames
          .filter(cacheName => cacheName.startsWith('minibar-') && cacheName !== STATIC_CACHE && cacheName !== DYNAMIC_CACHE && cacheName !== IMAGE_CACHE)
          .map(cacheName => {
    ...
    fetch(request)
      ...
      .catch(() => {
        return caches.match(request).then(cacheResponse => {
          return cacheResponse || caches.match('/').then(fallback => {
            return fallback || new Response('Offline - İnternet bağlantınızı kontrol edin', {
              status: 503,
