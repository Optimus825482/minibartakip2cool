...
// QR Kod Yazdır
function qrYazdir() {
    const imgSrc = $('#qrImage').attr('src');
    const odaNo = $('#qrOdaNo').text();
    ...
    printWindow.document.write(`
        ...
        <body>
            <h2>Oda ${odaNo}</h2>
            <img src="${imgSrc}" alt="QR Kod">
            <p>Minibar Dolum Talebi</p>
        </body>
        </html>
    `);
    ...
    $.ajax({
        ...
        success: function(response) {
            if (response.success) {
                ...
                $('#misafirMesajiModal').data('geri-donus-modal', geriDonusModal || null);
...
function misafirMesajiKaydet() {
    ...
    if (mesaj.length > 500) {
        showModalAlert('misafirMesajiAlert', 'error', 'Mesaj maksimum 500 karakter olabilir');
        return;
    }
...
$(document).ready(function() {
    $('#misafirMesajiInput').on('input', function() {
        ...
        if (len > 500) {
            $('#karakterSayaci').addClass('text-red-600 dark:text-red-400').removeClass('text-slate-500 dark:text-slate-400');
        } else {
            $('#karakterSayaci').removeClass('text-red-600 dark:text-red-400').addClass('text-slate-500 dark:text-slate-400');
        }
    });
});
...
function odaDuzenle(odaId, odaNo, katId, qrVar) {
    ...
    if (qrVar) {
        ...
        $.ajax({
            ...
            success: function(response) {
                if (response.success) {
                    $('#duzenleQrImage').attr('src', response.data.qr_image);
                    $('#qrKodBolumu').show();
                    $('#qrYokMesaji').hide();
                } else {
                    $('#qrKodBolumu').hide();
...
$(document).ready(function() {
    $('#odaDuzenleForm').on('submit', function(e) {
        ...
        $.ajax({
            ...
            success: function(response) {
                showModalAlert('odaDuzenleAlert', 'success', 'Oda başarıyla güncellendi');
            ...
            error: function(xhr) {
                const message = xhr.responseJSON?.message || 'Oda güncellenirken hata oluştu';
...
function qrYenile() {
    ...
    $.ajax({
        ...
        success: function(response) {
            if (response.success) {
                showModalAlert('odaDuzenleAlert', 'success', 'QR kod başarıyla yenilendi!');
        ...
        error: function(xhr) {
            if (xhr.status === 429) {
                showModalAlert('odaDuzenleAlert', 'error', 'Çok fazla deneme. Lütfen 1 dakika bekleyin.');
            } else {
...
function qrYazdirDuzenle() {
    const imgSrc = $('#duzenleQrImage').attr('src');
    const odaNo = $('#duzenleOdaNo').text();
...
function yeniOdaModal() {
    ...
    $('#yeniOdaKaydetBtn').show().prop('disabled', false).html('<svg class="h-4 w-4 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M8 7H5a2 2 0 00-2 2v9a2 2 0 002 2h14a2 2 0 002-2V9a2 2 0 00-2-2h-3m-1 4l-3 3m0 0l-3-3m3 3V4"></path></svg> Kaydet ve QR Oluştur');
    ...
    $('#yeniOdaForm').on('submit', function(e) {
        ...
        $.ajax({
            ...
            success: function(response) {
                ...
                showModalAlert('yeniOdaAlert', 'info', 'Oda eklendi, QR kod oluşturuluyor...');
                            ...
                            $.ajax({
                                ...
                                success: function(qrResponse) {
                                    if (qrResponse.success) {
                                        ...
                                        showModalAlert('yeniOdaAlert', 'error', 'QR kod oluşturulamadı: ' + qrResponse.message);
                                        $('#yeniOdaKaydetBtn').prop('disabled', false).html('<svg class="h-4 w-4 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M8 7H5a2 2 0 00-2 2v9a2 2 0 002 2h14a2 2 0 002-2V9a2 2 0 00-2-2h-3m-1 4l-3 3m0 0l-3-3m3 3V4"></path></svg> Kaydet ve QR Oluştur');
                                    }
                                },
                                error: function() {
                                    showModalAlert('yeniOdaAlert', 'error', 'QR kod oluşturulurken hata oluştu');
                                    $('#yeniOdaKaydetBtn').prop('disabled', false).html('<svg class="h-4 w-4 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M8 7H5a2 2 0 00-2 2v9a2 2 0 002 2h14a2 2 0 002-2V9a2 2 0 00-2-2h-3m-1 4l-3 3m0 0l-3-3m3 3V4"></path></svg> Kaydet ve QR Oluştur');
                                }
                            });
                        } else {
            ...
            error: function(xhr) {
                const message = xhr.responseJSON?.message || 'Oda eklenirken hata oluştu';
...
function qrYazdirYeni() {
    const imgSrc = $('#yeniOdaQrImage').attr('src');
