...
class DataCollector:
    ...
    def collect_all_metrics(self, save_features=True):
        ...
        try:
            ...
            total_count = stok_count + transaction_count + tuketim_count + dolum_count + zimmet_count + occupancy_count + talep_count + qr_count
    ...
    def collect_stok_metrics_incremental(self):
        ...
        try:
            ...
            # Tüm ürünlerin stok durumunu tek sorguda hesapla
            stok_durumu = self.db.session.query(
                StokHareket.urun_id,
                func.coalesce(func.sum(case((StokHareket.hareket_tipi == 'giris', StokHareket.miktar), else_=0)), 0).label('giris_toplam'),
                func.coalesce(func.sum(case((StokHareket.hareket_tipi == 'cikis', StokHareket.miktar), else_=0)), 0).label('cikis_toplam')
            ).group_by(StokHareket.urun_id).all()
            
            stok_map = {row.urun_id: (row.giris_toplam - row.cikis_toplam) for row in stok_durumu}
